...
The tool appends the provided `fact` to your context file in the user's home directory (`~/.terra-code/TERRA.md` by default). This filename can be configured via `contextFileName`.

Once added, the facts are stored under a `## Terra Added Memories` section. This file is loaded as context in subsequent sessions, allowing the CLI to recall the saved information.
