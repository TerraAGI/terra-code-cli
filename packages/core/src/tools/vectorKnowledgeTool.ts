...
Think of this as your INSTINCTIVE knowledge recall - like a senior developer who automatically thinks "let me check what we already know about this" before answering ANY question!`,
      Icon.LightBulb,
      {
        properties: {
          query: {
            description: 'The search query or topic to research in the knowledge base. Be specific but also try related terms, synonyms, and broader concepts to get comprehensive results.',
            type: 'string',
          },
          limit: {
  ...
  async execute(
    ...
    if (!terraApiKey || !terraUsername) {
      return {
        llmContent: 'Terra credentials not found. Terra credentials are automatically registered when you authenticate with Qwen/OpenAI/Gemini. Please authenticate first.',
        returnDisplay: 'Terra credentials not found. Terra credentials are automatically registered when you authenticate with Qwen/OpenAI/Gemini. Please authenticate first.',
      };
    }
    ...
    try {
      // Initial search
      const initialResult = await searchDocuments(query, collectionName, limit, terraApiKey);
      ...
      if (initialResults.length === 0) {
        ...
        for (const term of alternativeTerms.slice(0, 3)) {
          const altResult = await searchDocuments(term, collectionName, 3, terraApiKey);
        ...
        for (const concept of relatedConcepts.slice(0, 2)) {
          const conceptResult = await searchDocuments(concept, collectionName, 2, terraApiKey);
          if (conceptResult.success && conceptResult.results) {
            alternativeResults = alternativeResults.concat(conceptResult.results as SearchResult[]);
            searchAttempts++;
          }
        }
        ...
        for (const term of implementationTerms.slice(0, 2)) {
          const implResult = await searchDocuments(`${term} ${query}`, collectionName, 2, terraApiKey);
          if (implResult.success && implResult.results) {
            alternativeResults = alternativeResults.concat(implResult.results as SearchResult[]);
            searchAttempts++;
          }
        }
        ...
        for (const term of broaderTerms.slice(0, 2)) {
          const broadResult = await searchDocuments(term, collectionName, 2, terraApiKey);
          if (broadResult.success && broadResult.results) {
            alternativeResults = alternativeResults.concat(broadResult.results as SearchResult[]);
            searchAttempts++;
          }
        }
        
        if (alternativeResults.length > 0) {
          const uniqueAltResults = this.removeDuplicateResults(alternativeResults);
          return {
        ...
        return {
          ...
          returnDisplay: 'No results found despite comprehensive search strategies.',
        };
      }
      ...
      if (deep_search && initialResults.length > 0) {
        ...
        for (const term of keyTerms.slice(0, 4)) {
          const deepResult = await searchDocuments(term, collectionName, 3, terraApiKey);
        ...
        const relatedConcepts = this.extractRelatedConcepts(initialResults[0].content, query);
        for (const concept of relatedConcepts.slice(0, 3)) {
          const conceptResult = await searchDocuments(concept, collectionName, 2, terraApiKey);
          if (conceptResult.success && conceptResult.results) {
            deepResults = deepResults.concat(conceptResult.results as SearchResult[]);
          }
        }
        
        // Strategy 3: Search for implementation patterns and best practices
        const implementationTerms = this.extractImplementationTerms(initialResults[0].content);
        for (const term of implementationTerms.slice(0, 2)) {
          const implResult = await searchDocuments(`${term} ${query}`, collectionName, 2, terraApiKey);
          if (implResult.success && implResult.results) {
            deepResults = deepResults.concat(implResult.results as SearchResult[]);
          }
        }
   ...
   */
  private extractRelatedConcepts(content: string, _originalQuery: string): string[] {
  ...
  private generateBroaderSearchTerms(query: string): string[] {
    ...
    const generalConcepts = ['implementation', 'pattern', 'approach', 'method', 'solution'];
